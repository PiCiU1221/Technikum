<?php
	...
	//plik php sluzacy do wyswietlania danych wyszukiwania
...
<html>
    ...
    <body>
        ...
        <div class="main_part main_part_search">
            <div class="main_panel">
				...
				<div id="selected_post_display">
					<?php						
						...
						for ($x = $max_ID; $x >= 1; $x--) {
							$sql_query = "select count(*) as cntSearchPosts from Books where ID like '$x' and (Tytul like '%".$search_word."%' or Gatunek like '%".$search_word."%' or Autor like '%".$search_word."%')";
							$result = mysqli_query($conn,$sql_query);
							$row = mysqli_fetch_array($result);
							...
							if($count > 0) {
								$title_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Tytul FROM Books WHERE ID='".$x."'"));
    							$title = $title_s[0];
								$text_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Autor FROM Books WHERE ID='".$x."'"));
    							$text = $text_s[0];
								$author_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Gatunek FROM Books WHERE ID='".$x."'"));
    							$author = $author_s[0];

								$okladka_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Okladka FROM Books WHERE ID='".$x."'"));
    							$okladka = $okladka_s[0];
					...
					?>
					<script>
						document.getElementById('search_info').innerHTML = 'Znaleziono <?php echo $results_sum;?> wyników na wyszukanie "<?php echo $search_word;?>".';
					</script>
                </div>
