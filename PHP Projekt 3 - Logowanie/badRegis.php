    ...
    if ((isset($_POST['submit'])) && (!empty($_POST['submit'])) && (!empty($_POST['username_regis']))) {
        ...
        if($count > 0){
			...
			if(mysqli_query($conn, $sql)){
				header('location: afterRegis.php');
    			echo "Rekordy prowadzono pomyślnie.";
			}
			else {
...
<html>
    <head>
		...
		<script src="scriptBadRegis.js"></script>
		...
		<div class="login_box">
	    	<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
		    	...
		    	<div id="rejestracja">
					...
					<div class="group">
					<?php
        				echo "<span id='usedNickname'>Nazwa użytkownika jest już zajęta!</span>";
    				?>
					</div>
