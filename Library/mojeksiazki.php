<?php
	...
	//przerzucenie na glowna strone gdy ktos nie jest zalogowany
    ...
    <body>
	    <div class="top_strip">
			...
			<div class="login_box_main">
                ...
                <div id="log_out_box">
					<input id="mybooks_button" value="Moje książki">
        ...
        <div class="main_part">
			...
			<div class="main_panel_moje">
				...
				<?php					
					...
					for ($x = $max_ID; $x >= 1; $x--) {
						$sql_query = "select count(*) as cntSearchPosts from Borrowed where ID='".$x."' and Uzytkownik='".$username."'";
						$result = mysqli_query($conn,$sql_query);
						$row = mysqli_fetch_array($result);

						$count = $row['cntSearchPosts'];

						if($count > 0) {
							$title_s = mysqli_fetch_row(mysqli_query($conn,"SELECT book_id FROM Borrowed WHERE ID='".$x."'"));
    						$y = $title_s[0];

							$title_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Tytul FROM Books WHERE ID='".$y."'"));
    						$title = $title_s[0];
							$text_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Autor FROM Books WHERE ID='".$y."'"));
    						$text = $text_s[0];
							$author_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Gatunek FROM Books WHERE ID='".$y."'"));
    						$author = $author_s[0];
							$date_s = mysqli_fetch_row(mysqli_query($conn,"SELECT date FROM Borrowed WHERE ID='".$x."'"));
    						$date = $date_s[0];
							$date_oddania_s = mysqli_fetch_row(mysqli_query($conn,"SELECT date_oddania FROM Borrowed WHERE ID='".$x."'"));
    						$date_oddania = $date_oddania_s[0];
							$okladka_s = mysqli_fetch_row(mysqli_query($conn,"SELECT Okladka FROM Books WHERE ID='".$y."'"));
    						$okladka = $okladka_s[0];

							$results_sum = $results_sum + 1;
		
							echo '<div class="post_display_moje wrapping">
								<table>
									<tr>
										<th class="th_mojeksiazki">
											<img id="okladka_moje" src="'.$okladka.'">
										</th>
									</tr>
									<tr>
										<td class="td_mojeksiazki">
											<div id="post_display_title">'.$title.'</div>
											<div id="post_display_author">'.$text.'</div>
											<div id="post_display_genre">'.$author.'</div>
											<div id="post_display_genre">Data wypoż.: '.$date.'</div>';
											if ($date_oddania == $date) {
												echo '<div id="post_display_oddanie"><b>Nie oddano książki</b></div>';
											} else {
								
												echo '<div id="post_display_genre">Data oddania: '.$date_oddania.'</div>';
											} echo '
										</td>
									</tr>
								</table>
								<a href="http://www.ppie2018.s.zs1.stargard.pl/post.php?id='.$y.'"></a>
							</div>';
						}
					}
				?>
